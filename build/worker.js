...
async function onGet(request) {
  ...
  if (path.startsWith('/~_~_gdindex/resources/')) {
    ...
    const result = await gd.getMetaByPath(path, self.props.default_root_id);
...
async function handleRequest(request) {
  if (request.method === 'OPTIONS') // allow preflight request
    ...
    // return new Response(` url:${request.url} pathname:${request.pathname} headers:${JSON.stringify(Object.fromEntries(request.headers))} 0`, { status: 401 });
  
    // if (Object.fromEntries(request.headers).hasOwnProperty("authorization") || Object.fromEntries(request.headers).hasOwnProperty("WWW-Authenticate") ){
    //   return new Response(`fewfweu:${Object.fromEntries(request.headers)["authorization"]}`, { status: 500 });
    // }
    ...
    //   }
    //   const [user, pass] = parseBasicAuth(auth);
    //   return new Response(`auth： ${user} ${pass} ${user === self.props.user && pass === self.props.pass}`, { status: 200 });
    // }
  ...
  if (self.props.auth && !doBasicAuth(request)) {
    ...
    return unauthorized();
    // }else{
    // return new Response(`123:${Object.fromEntries(request.headers)["Authorization"]}`, {
    //   headers: {
    //     'WWW-Authenticate': 'Basic realm="goindex"',
    //     'Access-Control-Allow-Origin': '*',
    //     'Ac': Object.fromEntries(request.headers)["Authorization"]
    //   },
    //   status: 200
    // });
    // }

    // return new Response(`${Object.fromEntries(request.headers)["Authorization"]} || url:${request.url} pathname:${request.pathname} headers:${JSON.stringify(Object.fromEntries(request.headers))} 0`, { status: 401 });
  }
  ...
  // const updatedRequest = new Request(url.href, { 
  ...
  // ||  request.headers.get('x-lite') == 'true'
...
addEventListener('fetch', event => {
  ...
  event.respondWith(handleRequest(updatedRequest).catch(err => {
    console.error(err);
  ...
  // event.respondWith(
  //   new Response(`url:${updatedRequest.url} pathname:${updatedRequest.pathname} headers:${JSON.stringify(Object.fromEntries(updatedRequest.headers))} 0`, { status: 500 })
  //   );
